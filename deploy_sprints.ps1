...
param(
    [switch]$DryRun = $false,  # Use -DryRun to see what would happen without executing
    [switch]$Force = $false     # Use -Force to skip confirmations
)
...
# Function to generate random timestamp within date range
function Get-RandomTimestamp {
    ...
    $randomHours = Get-Random -Minimum 8 -Maximum 20  # Working hours
...
# Function to check if files/folders exist
...
# Change remote to Dummy repository
if (-not $DryRun) {
...
foreach ($sprintNum in 1..5) {
    ...
    # Generate 10-12 commits for this sprint
    ...
    # Divide files into groups for commits
    $fileGroups = @()
    ...
    # Generate commit messages for this sprint
    ...
    # Create commits with random timestamps
    ...
    $timestamps = $timestamps | Sort-Object  # Ensure chronological order
